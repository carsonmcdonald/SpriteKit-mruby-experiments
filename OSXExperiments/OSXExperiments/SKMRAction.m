...
+ (void)registerModule:(mrb_state *)mrb withRootModule:(struct RClass *)skmrModule
{
    ...
    mrb_define_class_method(mrb, skmrActionClass, "create_sequence", create_sequence, MRB_ARGS_REQ(1));
}
...
static mrb_value create_fade_in(mrb_state *mrb, mrb_value obj)
{
    mrb_float d;
    ...
    SKAction *action = [SKAction fadeInWithDuration:d];
...
static mrb_value create_fade_out(mrb_state *mrb, mrb_value obj)
{
    mrb_float d;
    mrb_get_args(mrb, "f", &d);
...
static mrb_value create_sequence(mrb_state *mrb, mrb_value obj)
{
    mrb_value animations;
    mrb_get_args(mrb, "A", &animations);
    ...
    SKAction *action = [SKAction sequence:animationGroup];
